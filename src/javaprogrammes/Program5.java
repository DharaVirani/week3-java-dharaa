...
//    It should return true if the number is a palindrome number otherwise it should return false.
    ...
    int reverse=0;                      //declared variable reverse to 0
    int originalNumber= number;         //storing the number declared in a variable to not change the value which was already there
    while(number!=0 || number>0){       //Repeat steps until the number is not equal to (or greater than) zero.
    int lastDigit= number %10;          // Store the last digit to some variable say lastDigit = num % 10
    reverse= (reverse *10) + lastDigit;//To increase place value multiply the reverse variable by 10 e.g. reverse = reverse * 10.  Add lastDigit to reverse.
        number/=10;                   //remove the last digit of num. To remove the last digit  divide number by 10.
    }return originalNumber==reverse;  //comparing a reversed number  with the starting number (parameter).
    ...
    public static void main(String[] args) {       //main method
        ...
        scanner.close();
        System.out.println(isPalindrome(number));   //calling the static method
    }
}
